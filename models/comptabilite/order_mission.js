...
class order_mission {
  ...
  static selectListOrdreMissionByDate(e) {
          ...
          return conn
            .execute(
              ` SELECT 1000000 AS AD_ORG_ID,
                ...
                br.XX_DAYNUMBER,
                TO_CHAR(br.STARTDATE, 'DD/MM/YYYY') as StarDate,
                br.ENDDATE,
                br.DESCRIPTION,
                SUM(m.VOLUME)                               AS co,
                SUM(m.WEIGHT)                               AS cv,
              ...
              WHERE TRUNC(DATEREPORT) >= TO_DATE(:date1, 'DD/MM/YYYY')
                AND TRUNC(DATEREPORT) <= TO_DATE(:date2, 'DD/MM/YYYY')
                AND tr.C_BPARTNER_ID = :partnerId
              AND br.processed='Y'
              and br.ISPAID='N'
  ...
  static selectListOrdreMission(e) {
    return new Promise((resolve, reject) => {
      connect
        .then(function (conn) {
          return conn
    ...
    switch (indice) {
      case 3:
        connect
          .then(function (conn) {
            return conn
              .execute(" select max(xx_rowid)+1 from R_Request ")
              .then(function (result) {
                cb(result.rows);
                //console.log(result.rows);
              })
      ...
      case 4:
        connect
          .then(function (conn) {
            return conn
              .execute(" select R_Request_ID from R_Request  where xx_rowid=:xx_rowId", { xx_rowId: e.xx_rowId })
              .then(function (result) {
                cb(result.rows);
                //console.log(result.rows);
              })
