  ...
  static async selectProductToDtQRN(query) {
    try {
      ...
      SELECT ls.VALUE AS LOCATORSOURCE,
             ...
             p.NAME || '   ' || P.VERSIONNO AS NAME,
             CASE WHEN SLC.VALUE='Q' THEN 'Q' ELSE ' ' END AS QUOTA,
             P.ISQR AS QR,
             aPPA.VALUENUMBER AS PPA,
             aSHP.VALUENUMBER AS SHP,
             ASI.LOT,
             TO_CHAR(asi.GUARANTEEDATE, 'DD/MM/YYYY') AS GUARANTEEDATE,
             BP.NAME AS Fournisseur,
             SUM(il.MOVEMENTQTY) AS MOVEMENTQTY,
             SUM(il.Orig_QtyEntered) AS OrigQtyEntered,
             MAX(il.QtyMovemented) AS QtyMovemented,
             ...
             LISTAGG(il.M_InoutLine_ID, ',') 
               WITHIN GROUP (ORDER BY ls.VALUE, GSRG.NAME, p.NAME, P.VERSIONNO) AS mInoutlineIdList,
             il.XX_CONTROLEURTR_ID,
             il.XX_PREPARATEURTR_ID,
             UP.NAME AS PREPARATEUR,
             UC.NAME AS CONTROLEUR,
      ...
      FROM M_InoutLine il
      INNER JOIN M_Product p ON il.m_product_id = P.M_PRODUCT_ID
      LEFT JOIN AD_USER UP ON UP.AD_USER_ID = IL.XX_PREPARATEURTR_ID
      LEFT JOIN AD_USER UC ON UC.AD_USER_ID = IL.XX_CONTROLEURTR_ID
      LEFT JOIN XX_Forme FM ON FM.XX_Forme_ID = P.XX_Forme_ID
      INNER JOIN XX_SalesContext SLC ON SLC.XX_SalesContext_ID = P.XX_SalesContext_ID
      INNER JOIN M_Attributesetinstance asi ON asi.M_ATTRIBUTESETINSTANCE_ID = il.M_ATTRIBUTESETINSTANCE_ID
      LEFT JOIN M_AttributeInstance aPPA 
        ON aPPA.m_attributesetinstance_id = il.m_attributesetinstance_id 
       AND aPPA.m_attribute_id = 1000503
      LEFT JOIN M_AttributeInstance aSHP  
        ON aSHP.m_attributesetinstance_id = il.m_attributesetinstance_id 
       AND aSHP.m_attribute_id = 1000506
      LEFT JOIN M_AttributeInstance fr   
        ON fr.m_attributesetinstance_id = il.m_attributesetinstance_id  
       AND fr.m_attribute_id = 1000508
      LEFT JOIN C_BPARTNER BP ON BP.C_BPARTNER_ID = fr.VALUENUMBER
      INNER JOIN M_InOutGroupLine ILG ON IL.M_InOutGroupLine_ID = ILG.M_InOutGroupLine_ID
      INNER JOIN M_InOutGroup IG ON IG.M_InOutGroup_ID = ILG.M_InOutGroup_ID
      INNER JOIN M_Inout I ON I.M_INOUT_ID = il.M_INOUT_ID
      INNER JOIN C_DocType DOC ON I.C_DocType_ID = DOC.C_DocType_ID
      ...
      WHERE i.docstatus IN ('IP', 'CO', 'CL','DR')
        ...
        AND i.XX_ISDEJASERVI = 'N'
        AND I.ISQR = :isQr
        AND IG.MOVEMENTDATE = TO_DATE(:movementDate, 'DD-MM-YYYY')
        ...
        AND NOT EXISTS (
          ...
          WHERE IL.M_INOUTGROUPLINE_ID IS NULL
            ...
            AND i.MOVEMENTDATE <= TO_DATE(:movementDate, 'DD-MM-YYYY')
            AND I.ISQR = :isQr
        )
      GROUP BY ls.VALUE, GSRG.NAME, p.NAME, P.VERSIONNO, 
               SLC.VALUE, P.ISCHER, P.ISQR, aPPA.VALUENUMBER, 
               ASI.LOT, asi.GUARANTEEDATE, aSHP.VALUENUMBER, 
               BP.NAME, SRG.C_SALESREGIONGROUP_ID, ls.m_locator_id, 
               p.m_product_id, p.XX_ExclueControleLot, 
               p.XX_ExclueControlePPA, p.XX_ExclueControleDate, 
               il.XX_CONTROLEURTR_ID, il.XX_PREPARATEURTR_ID, 
               UP.NAME, UC.NAME, P.XX_Forme_ID, FM.DESCRIPTION
